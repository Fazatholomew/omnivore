...
def dataframe():
    ...
    return DataFrame(
        data={
            "Name": ["Jimmy Dummy", "Testy Test"],
            "HEA Status": ["Scheduled", "Completed"],
        }
    )
...
def sf():
    ...
    with patch("omnivore.utils.salesforce.Salesforce") as MockSf:
        ...
        MockSf.return_value.Account.create.return_value = Create(
            errors=[], id="new account", success=True
        )
...
async def test_upload_to_salesforce(sf):
    ...
    data: list[Record_Find_Info] = [
        Record_Find_Info(
            acc=Account(PersonEmail="aldoglean@gmail.com", LastName="yeah"),
            opps=[
                Opportunity(
                    ID_from_HPC__c="259292987",
                    Don_t_Omnivore__c=True,
                    tempId="test test test",
                )
            ],
        )
    ]
    ...
    await omni.start_upload_to_salesforce(data, NEEECO_ACCID)
    assert (
        sf.return_value.Opportunity.update.call_args_list[0][0][0]
        == "0063i00000DEbCsAAL"
    )
    assert (
        sf.return_value.Opportunity.update.call_args_list[0][0][1]["StageName"]
        == "Completed"
    )
    # Test creating
    data: list[Record_Find_Info] = [
    ...
    await omni.start_upload_to_salesforce(data, NEEECO_ACCID)
    assert (
        sf.return_value.Opportunity.create.call_args_list[0][0][0]["StageName"]
        == "Completed"
    )
            ...
            with patch("omnivore.app.load") as mock_load:
                with patch("omnivore.app.dump") as mock_dump:
                    ...
                    mock_dump.assert_called_with(
                        set(["test test"]), mocked_open.return_value
                    )
