import pytest
from omnivore.utils.salesforce import SalesforceConnection, Record_Find_Info, Account, Opportunity, Create
...
def test_find_records_ids_search(sf: SalesforceConnection):
    ...
    input_data = Record_Find_Info(acc=Account(LastName='Test'), opps=[Opportunity(
        All_In_Energy_ID__c='<|>0063i00000CwA9NAAV<|>', CloseDate='date'), Opportunity(ID_from_HPC__c='259292987', CloseDate='date')])
...
def test_find_records_phone_search(sf: SalesforceConnection):
    ...
    input_data = Record_Find_Info(acc=Account(LastName='Test', Phone='6172178302'), opps=[Opportunity(
        CloseDate='date'), Opportunity(CloseDate='date'), Opportunity(CloseDate='date')])
    output_data = sf.find_records(input_data)
...
def test_find_records_email_search(sf: SalesforceConnection):
    ...
    # Set the HPC ID
    input_data = Record_Find_Info(acc=Account(LastName='Test', PersonEmail='nolviagutierrez1977@gmail.com'), opps=[Opportunity(
